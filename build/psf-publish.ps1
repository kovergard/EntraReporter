...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'EntraReporter' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\EntraReporter" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the EntraReporter module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\EntraReporter"
