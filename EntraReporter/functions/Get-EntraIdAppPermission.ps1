...
#
# TODO: Add reference to app_Permissions_inventory_GraphAPI.ps1 by @michev.
#
...
foreach ($SP in $SPs) {
	...
	try {
		...
		$memberOfGroups = (($res.Content | ConvertFrom-Json).Value | Where-Object { $_.'@odata.type' -eq "#microsoft.graph.group" }).DisplayName -join ";"
		$memberOfRoles = (($res.Content | ConvertFrom-Json).Value | Where-Object { $_.'@odata.type' -eq "#microsoft.graph.directoryRole" }).DisplayName -join ";"
	}
	...
	try {
		...
		if (!$appRoleAssignments) { Write-Verbose "No application permissions to report on for SP $($SP.id), skipping..." }
			...
			$objPermissions.'Last modified (application)' = (Get-Date($appRoleAssignments.CreationTimestamp | Select-Object -Unique | Sort-Object -Descending | Select-Object -First 1) -Format g)
...
$output | Select-Object * -ExcludeProperty Number | Export-Csv -nti -Path "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_GraphAppInventory.csv"
